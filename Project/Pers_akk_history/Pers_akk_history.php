...
<head>
    ...
    <title>История</title>
    <link rel="stylesheet" href="Pers_akk_history_Style.css">
</head>
<body>
<a id="top"></a>
<div class="Upper">
        ...
        <div class="bread">
            <ul>
                <li>
                    ...
                    <a href="#" style="background-color: red">История</a>
                </li>
            </ul>
        </div>
                                                <!--Конец шапки-->
        ...
        <div class="content">
            <div class="left">
                <ul class="pers_menu">
                    ...
                    <li> <a href="#" style="background-color: red">История</a> </li>
                    <li> <a href="../Pers_akk_bonus/Pers_akk_bonus.php">Бонусы</a> </li>
                    <li> <a href="Pers_akk_history.php">Настройки</a> </li>
                </ul>
            </div>
            <div class="right">
                <?php
                $history=R::findAll('history','user=?',array($_SESSION['logged_user']['name']));
                if (!empty($history))
                {
                    echo '<table class="tab" width="60%" cellpadding="0" border="1">';
                    foreach ($history as $h_bd)
                    {
                        echo '<tr>';
                        echo '<th width="15%">Дата заказа</th>';
                        echo '<th width="15%">Номер заказа</th>';
                        echo '<th width="15%">Сумма заказа</th>';
                        echo '</tr>';
                        echo '<tr>';
                        echo '<td>'.$h_bd->date.'</td>';
                        echo '<td>'.$h_bd->number.'</td>';
                        echo '<td>'.$h_bd->total.'</td>';
                        echo '</tr>';
                    }
                    echo '</table>';
                }
